...
const Header = () => {
  ...
  let cartItems =useSelector(str=>str.cart.items);
  ...
  const handlelogout = async () => {
    ...
    if (data.status === 200) {
      setLogin(false);
        ...
        localStorage.removeItem("resultofuser");
        navigate("/");
    } else {
        navigate("/*");
    }
  ...
  return (
    <>
      <div className="d-flex justify-content-around header shadow-sm"style={{zIndex:1}}>
        <div>
        <Link to="/">
          <img className="logo py-2" src={swiggy} alt="logo" />
        </Link>&nbsp;&nbsp;
        <i className="fa-solid fa-location-dot"></i>
        <select value={a.selectedoption} onChange={e=>a.setSelectedoption(e.target.value)} style={{border:"none"}}>
        ...
        <div className="d-flex justify-content-center align-items-center nav-items px-2">
          ...
          <span className="fw-bold signin" style={{cursor:"pointer"}} onClick={handlelogout}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="6 0 12 24"
              height="19"
            ...
            &nbsp;Logout
          </span> :   <Link to={'/login'}  style={{textDecoration:"none",color:"black","cursor":"pointer"}}>
          <span className="fw-bold signin">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="6 0 12 24"
              height="19"
          ...
          <div className="d-flex justify-content-center align-items-center fw-bold position-relative">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="-1 0 37 32" height="20" width="20" stroke="#3d4152" strokeWidth="3" fill="#48C479" className="cart-nonempty"><path d="M4.438 0l-2.598 5.11-1.84 26.124h34.909l-1.906-26.124-2.597-5.11z"></path></svg>
          <span className="position-absolute text-white">{cartItems?.length}</span>
          </div>
